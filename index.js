    ...
    if (message.body.match(/[a-z0-9]{24}/g)) {
        axios.post('https://api-kjr.kampustsl.id/mobile/whatsapp', {from: message.from, message: message.body}, {
            ...
            if (response.data.reply) {
                response.data.messages.forEach(async (item) => {
                    if (item.type === 'text')
                        await client.sendText(message.from, item.message);
                    else if (item.type === 'image')
                        await client.sendImage(message.from, item.message, item.label+'.png', item.label);
                })
            }
        })
    }
